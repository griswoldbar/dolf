...
Given(/#{capture_model} has (.*?): (.*?)$/) do |name, collection_name, collection|
  parent = model!(name)
  child_type = collection_name.singularize
  children = collection.split(",").map do |x|
    x.strip!
    model!("#{child_type}: \"#{x}\"")
  end
  parent.send("#{collection_name}=", children)
end
...
Given(/^#{capture_model} exists with (\d+) (.*?)$/) do |name, count, collection_name|
  model = create_model(name)
  collection = count.to_i.times.map { create_model(collection_name.singularize) }
  model.send("#{collection_name}=", collection)
end
