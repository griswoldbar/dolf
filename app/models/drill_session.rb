class DrillSession < ActiveRecord::Base
  ...
  scope :next_drill_session, ->(ds) { joins('inner join drills on drills.id = drill_sessions.drill_id').
    where(workout_session_id: ds.workout_session_id).
    where('drills.position = ?', ds.position + 1)
  }
  ...
  def previous_session
    @previous_session ||= DrillSession.complete.where('drill_sessions.id != ?', self.id).where(drill: self.drill).includes(:drill).last
  end
